...
export default function PdfViewer({ document, activeChunkText = "", highlightTerms = [] }) {
  ...
  const initialPage = useMemo(() => {
    if (!activeChunkText || pages.length === 0) return 0;
    const match = pages.findIndex((c) => c.text?.includes(activeChunkText));
      ...
      <div className="flex items-center justify-between gap-3 border-b border-gray-200 dark:border-gray-700 px-4 py-3">
        <div className="min-w-0">
          ...
          <p className="text-xs text-gray-500 dark:text-gray-400">
            Chunk {pageIndex + 1} of {pages.length || 1}
          </p>
        </div>
        <div className="flex items-center gap-2">
          ...
          <button
            type="button"
            onClick={() => setPageIndex((c) => Math.min(pages.length - 1, c + 1))}
